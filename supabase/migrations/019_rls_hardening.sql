...
--
-- Each section is guarded with `to_regclass` so the migration is safe to
-- apply even when an earlier migration that creates the target table
-- hasn't been pushed yet (e.g. fresh staging DB). Missing tables are
-- skipped with a NOTICE — re-run after the prerequisite migration.
-- ============================================
...
BEGIN
  IF to_regclass('public.reservations') IS NULL THEN
    RAISE NOTICE 'Skipping reservations RLS — table does not exist (apply migration 012 first).';
    RETURN;
  END IF;

  EXECUTE 'DROP POLICY IF EXISTS "Users can view their reservations" ON public.reservations';
  EXECUTE 'DROP POLICY IF EXISTS "Clients can create reservations" ON public.reservations';
  EXECUTE 'DROP POLICY IF EXISTS "Users can update their reservations" ON public.reservations';
...
BEGIN
  IF to_regclass('public.direct_messages') IS NULL THEN
    RAISE NOTICE 'Skipping direct_messages RLS — table does not exist (apply migration 013 first).';
    RETURN;
  END IF;

  EXECUTE 'DROP POLICY IF EXISTS "Users can view their direct messages" ON public.direct_messages';
  EXECUTE 'DROP POLICY IF EXISTS "Users can send direct messages" ON public.direct_messages';
  EXECUTE 'DROP POLICY IF EXISTS "Users can update received messages" ON public.direct_messages';
  EXECUTE 'DROP POLICY IF EXISTS "direct_messages_select" ON public.direct_messages';
  EXECUTE 'DROP POLICY IF EXISTS "direct_messages_insert" ON public.direct_messages';
  EXECUTE 'DROP POLICY IF EXISTS "direct_messages_update" ON public.direct_messages';
...
DO $$
BEGIN
  IF to_regclass('public.livraisons') IS NULL THEN
    RAISE NOTICE 'Skipping livraisons RLS — table does not exist (apply migration 016 first).';
    RETURN;
  END IF;

  EXECUTE 'DROP POLICY IF EXISTS "Users can view their livraisons" ON public.livraisons';
  EXECUTE 'DROP POLICY IF EXISTS "Clients can create livraisons" ON public.livraisons';
  EXECUTE 'DROP POLICY IF EXISTS "Parties can update livraisons" ON public.livraisons';
  EXECUTE 'DROP POLICY IF EXISTS "livraisons_select" ON public.livraisons';
  EXECUTE 'DROP POLICY IF EXISTS "livraisons_insert" ON public.livraisons';
  EXECUTE 'DROP POLICY IF EXISTS "livraisons_update" ON public.livraisons';
...
DO $$
BEGIN
  IF to_regclass('public.hebergement_reservations') IS NULL THEN
    RAISE NOTICE 'Skipping hebergement_reservations RLS — table does not exist (apply migration 014 first).';
    RETURN;
  END IF;

  EXECUTE 'DROP POLICY IF EXISTS "client_select_own_heb_reservations" ON public.hebergement_reservations';
  EXECUTE 'DROP POLICY IF EXISTS "hebergeur_select_own_heb_reservations" ON public.hebergement_reservations';
  EXECUTE 'DROP POLICY IF EXISTS "client_insert_heb_reservation" ON public.hebergement_reservations';
  EXECUTE 'DROP POLICY IF EXISTS "hebergeur_update_heb_reservation_status" ON public.hebergement_reservations';
  EXECUTE 'DROP POLICY IF EXISTS "hebergement_reservations_select" ON public.hebergement_reservations';
  EXECUTE 'DROP POLICY IF EXISTS "hebergement_reservations_insert" ON public.hebergement_reservations';
  EXECUTE 'DROP POLICY IF EXISTS "hebergement_reservations_update" ON public.hebergement_reservations';
...
-- carries — auth.uid() is null/wrong under TPA. storage.objects always
-- exists in Supabase, no guard needed.
...
BEGIN
  IF to_regclass('public.notifications') IS NULL THEN
    RAISE NOTICE 'Skipping notifications RLS — table does not exist (apply migration 002 first).';
    RETURN;
  END IF;
