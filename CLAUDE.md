...
- **Styling**: NativeWind v4 (TailwindCSS 3.4) + `global.css`
...
│           ├── trajets.tsx       # Gestion trajets (chauffeur)
│           ├── mes-hebergements.tsx  # Gestion hebergements (hebergeur)
...
│   ├── hebergementsStore.ts      # CRUD hebergements (hebergeur)
...
│   ├── supabaseHebergements.ts   # CRUD hebergements
...
- **Setup**: `jest.setup.js` (mocks: AsyncStorage, expo-secure-store, expo-network, expo-notifications, expo-device, supabase, supabaseProfile, supabaseNotifications, supabaseTrajets, supabaseHebergements)
